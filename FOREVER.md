...
| **Find button** | Parent `WorldMapFrame`, overlay strata like camp pins, anchored to `ScrollContainer` bottom-right (inset so the quest-log hide tab stays clickable). Not a scroll-container child — those draw under the parchment. | Confirm button still visible at zone + continent + world; hide-log tab still clickable. |
